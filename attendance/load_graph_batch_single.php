...
var chart = new CanvasJS.Chart("chartContainer",
{
  ...
  title:{
    text: "Batch:<?php $runBatchCode = batchDetail($batch);$dataShortCode = mysqli_fetch_assoc($runBatchCode);echo  $dataShortCode['batch_short_name']?> Attendance (Offline)"},
  ...
  for($inputDate = 0; $inputDate < $datesCount; $inputDate++){
    ...
    for($datasubwise = 0; $datasubwise < $newAllClassInBatchcount; $datasubwise++ ){
        ...
        if($datesArray[$inputDate] == $newAllClassInBatch[$datasubwise]['class_date']){?>
           ...
           { label: "<?php   $orgDate = "$datesArray[$inputDate]";  echo $newDate = date("d/m", strtotime($orgDate)); ?>", sub: "<?php echo $newAllClassInBatch[$datasubwise]['subject']; ?>", y: <?php echo $newAllClassInBatch[$datasubwise]['probabilityNoOfStudent']; ?>, indexLabel : "<?php echo $newAllClassInBatch[$datasubwise]['probabilityNoOfStudent']; ?>"},
