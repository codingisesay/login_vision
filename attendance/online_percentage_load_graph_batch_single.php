...
var chart = new CanvasJS.Chart("chartContainer",
{
  ...
  title:{
    text: "Batch:<?php $runBatchCode = batchDetail($batch);$dataShortCode = mysqli_fetch_assoc($runBatchCode);echo  $dataShortCode['batch_short_name']?> Attendance (Online%)"},
    ...
    toolTip: {
        content: "{label}<?php echo"<br>";?>{sub}: {y}"
    },
...
//     $subject = $all_class_faculty_array_unique_reindex[$graph][0];
    ...
    FROM checklist_record LEFT JOIN attendance_assignment_record 
    ON checklist_record.checklist_id =  attendance_assignment_record.checklist_id 
    WHERE checklist_record.batch LIKE '%$batch%' AND (checklist_record.checklist_type = 'Class' OR checklist_record.checklist_type = 'Offline') AND checklist_record.class_date between '$from' and '$to'";
  ...
  for($inputDate = 0; $inputDate < $datesCount; $inputDate++){
    ...
    for($datasubwise = 0; $datasubwise < $newAllClassInBatchcount; $datasubwise++ ){
        ...
        if($datesArray[$inputDate] == $newAllClassInBatch[$datasubwise]['class_date']){?>
           ...
           { label: "<?php   $orgDate = "$datesArray[$inputDate]";  echo $newDate = date("d/m", strtotime($orgDate));   ?>", sub:"<?php echo $newAllClassInBatch[$datasubwise]['subject']; ?>", y: <?php echo $newAllClassInBatch[$datasubwise]['percentageOfSelectedBatchAtt']; ?>, indexLabel : "<?php echo $newAllClassInBatch[$datasubwise]['percentageOfSelectedBatchAtt']."%"; ?>"},
